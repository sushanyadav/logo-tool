...
export async function POST(request: NextRequest) {
  try {
    ...
    const padding = parseInt(formData.get("padding") as string) || 2;
    ...
    const targetPadding = Math.min(Math.max(padding, 0), 10);
    ...
    // Process the image
    const processedBuffer = await processLogo(buffer, targetHeight, targetPadding);
